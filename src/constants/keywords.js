    ...
    const loadSuggestionLibraryJson = async () => {
        ...
        try {
            if (typeof chrome !== 'undefined' && chrome?.runtime?.getURL) {
                candidates.push(
                    chrome.runtime.getURL('suggestions-library.json'),
                );
                candidates.push(
                    chrome.runtime.getURL('suggestions-library-example.json'),
                );
            }
        } catch (e) {
            // Ignora
        ...
        for (const url of candidates) {
            try {
                ...
                if (normalized) {
                    window.ChatSuggestions.constants.SUGGESTION_LIBRARY =
                        normalized;
                    console.info('[Chat Suggestions] Biblioteca carregada', {
                        url,
