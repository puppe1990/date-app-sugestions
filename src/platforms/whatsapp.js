(() => {
    ...
    registry.register('whatsapp', {
        ...
        otherPersonNameSelector:
            '#pane-side [role="row"][aria-selected="true"] span[title], #pane-side [aria-selected="true"] span[title], #main header span[title], header span[title], #pane-side > div:nth-child(2) > div > div > div:nth-child(1) > div > div > div > div._ak8l._ap1_ > div._ak8o > div._ak8q > div > div > span',
        profileContainerSelector: '#main header',
        messageReaderConfig: {
            messageSelector:
                'div.copyable-text[data-pre-plain-text], div.message-in, div.message-out, div[data-testid="msg-container"], div[data-pre-plain-text]',
            textSelector:
                'span[data-testid="selectable-text"], span.selectable-text.copyable-text span, span[dir="auto"]',
            senderSelector: null,
            ...
            textResolver: (node) => {
                try {
                    if (
                        node &&
                        node.matches &&
                        node.matches('span[data-testid="selectable-text"]') &&
                        node.textContent
                    ) {
