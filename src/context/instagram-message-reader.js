(() => {
    ...
    const getOtherPersonNameFromDom = () => {
        try {
            ...
            if (headingTitle) {
                const text =
                    headingTitle.getAttribute('title') ||
            ...
            if (conversationHeader) {
                const ariaLabel =
                    conversationHeader.getAttribute('aria-label') || '';
                const normalized = normalizeText(
                    ariaLabel.replace(/Conversation with/i, ''),
                );
                if (normalized) return normalized;
            }

            const usernameCandidates = Array.from(
                document.querySelectorAll('span.x1lliihq.x193iq5w'),
            );
    ...
    const extractThreadsFromPayload = (payload) => {
        ...
        const walk = (node) => {
            ...
            if (
                node.ig_dm_thread_v2 &&
                typeof node.ig_dm_thread_v2 === 'object'
            ) {
    ...
    const getThreadId = (thread) =>
        normalizeId(thread?.thread_id || thread?.threadId || '');
    ...
    const pickThreadForActiveChat = (threads = []) => {
        ...
        return (
            threads.find(
                (thread) => normalizeId(getThreadId(thread)) === activeThreadId,
            ) || null
        );
    };
    ...
    const buildMessageFromItem = (item, thread) => {
        ...
        const isOutgoing =
            typeof item?.is_sent_by_viewer === 'boolean'
                ? item.is_sent_by_viewer
                : Boolean(
                      thread?.viewer_id &&
                      (item?.user_id === thread.viewer_id ||
                          item?.sender_id === thread.viewer_id),
                  );
    ...
    const updateStateFromThread = (thread) => {
        ...
        items.forEach((item) => {
            const id =
                item?.item_id || item?.client_context || item?.timestamp || '';
    ...
    if (
        !window.__bcsInstagramFetchIntercepted &&
            ...
            if (isGraphqlUrl(resource)) {
                return originalFetch.apply(this, args).then((response) => {
                    ...
                    clonedResponse
                        ...
                        .then((data) => {
                            try {
                                const threads = extractThreadsFromPayload(
                                    data?.data || data,
                                );
                                const activeThread =
                                    pickThreadForActiveChat(threads);
                                const selectedThread =
                                    activeThread || pickBestThread(threads);
    ...
    class InstagramMessageReader {
        ...
        read(container) {
            const captured = Array.isArray(state.lastMessages)
                ? state.lastMessages
                : [];
        ...
        readFromDom(container) {
            ...
            const logContainer = root.querySelector
                ? root.querySelector('main [role="log"]') ||
                  root.querySelector('[role="log"]')
                : null;
            const containerRect =
                logContainer && logContainer.getBoundingClientRect
        ...
        parseDomNode(node, containerRect) {
            ...
            candidates.forEach((el) => {
                ...
                if (
                    el.querySelector &&
                    el.querySelector('span[dir="auto"], div[dir="auto"]')
                )
                    return;
            ...
            return {
                sender:
                    direction === 'out' ? 'Voce' : otherPersonName || 'Outro',
                text,
                direction,
                type: 'text',
            };
        ...
        resolveDomDirection(node, containerRect) {
            ...
            const bubble =
                node.querySelector('span[dir="auto"], div[dir="auto"]') || node;
            const bubbleStyle =
                bubble && window.getComputedStyle
                    ? window.getComputedStyle(bubble)
                    : null;
            if (bubbleStyle) {
                if (bubbleStyle.marginLeft === 'auto')
                    return { direction: 'out', source: 'margin-left' };
                if (bubbleStyle.marginRight === 'auto')
                    return { direction: 'in', source: 'margin-right' };
            }
            ...
            for (let i = 0; i < 6 && el; i += 1) {
                ...
                if (style && style.display && style.display.includes('flex')) {
                    ...
                    if (
                        justify.includes('flex-end') ||
                        justify.includes('end')
                    ) {
                        return { direction: 'out', source: 'flex-justify' };
                    }
                    if (
                        justify.includes('flex-start') ||
                        justify.includes('start')
                    ) {
        ...
        isSystemText(text) {
            ...
            if (lower === 'seen' || lower === 'sent' || lower === 'delivered')
                return true;
    ...
    window.ChatSuggestions.createInstagramMessageReader =
        createInstagramMessageReader;
})();
